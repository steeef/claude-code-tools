...
## Unify Continue Behavior Across All Entry Points

**Session ID:** 1b3af010-6149-4e9a-8736-dbfd35ba2a5b

**Files Modified:**
- `claude_code_tools/session_utils.py` - Added `is_agent_available()`,
  `display_lineage()`, `continue_with_options()`; deleted `execute_continue_action()`
- `claude_code_tools/claude_continue.py` - Added `precomputed_exports` parameter
- `claude_code_tools/codex_continue.py` - Added `precomputed_exports` parameter
- `claude_code_tools/aichat.py` - Updated `continue` command to use unified helper
- `claude_code_tools/find_session.py` - Use `continue_with_options()`
- `claude_code_tools/find_claude_session.py` - Use `continue_with_options()`
- `claude_code_tools/find_codex_session.py` - Use `continue_with_options()`
- `claude_code_tools/session_menu_cli.py` - Use `continue_with_options()`

**Files Created:**
- `tests/test_continue_flow.py` - 11 tests for agent availability and parameter handling

**Description:**

Unified continue behavior across all entry points (`aichat continue`, `aichat menu`,
`aichat find*`). Key improvements:

1. **Proper timing**: Lineage displayed BEFORE prompting for options (user makes
   informed decisions)
2. **Agent availability check**: Only offer cross-agent choice if other agent
   is actually available (checks command in PATH or config dir exists)
3. **CLI flags**: `--agent` and `--prompt` skip their respective interactive prompts

**Reference:** `issues/021-20251124-unify-continue-behavior.md`
